...
async function refreshAnimalStats() {
  try {
      ...
      const data = await response.json();
      setAnimalStats(data.animals); // Mettre à jour le state avec les données des animaux (à adapter selon votre contexte)
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
      ...
      await addAnimalViews(animalId);
      console.log('Vue ajoutée avec succès.');

      // Rafraîchir les statistiques des animaux
      await refreshAnimalStats();
      console.log('Statistiques des animaux rafraîchies.');
